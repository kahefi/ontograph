...
var _ = Describe("BlazegraphStore", func() {
	var graphUri string
	var graph *BlazegraphStore
	var testTriples []Triple

	BeforeEach(func() {
		// Initialize graph store
		graphUri = fmt.Sprintf("https://www.ontograph.com/test-%s", shortuuid.New())
		graph = endpoint.NewBlazegraphStore(graphUri, testNamespace)
		...
		err = graph.AddTriplesUnchecked(testTriples)
		Expect(err).NotTo(HaveOccurred())
	})
